...
const notificationQueue = new Queue('notification processing', process.env.REDIS_URL || 'redis://localhost:6379');
...
class NotificationService {
  ...
  static async sendEmail({ to, subject, html, templateId, templateData, organizationId, metadata }) {
    try {
      ...
      if (templateId || templateData?.type) {
        ...
        if (template) {
          emailContent.subject = this.fillTemplate(template.subject, templateData);
      ...
      return { success: true, messageId: info.messageId, logId: notificationLog._id };
